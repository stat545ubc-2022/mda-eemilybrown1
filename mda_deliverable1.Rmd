...
1. Which neighbourhoods have the highest tree species diversity?
    - Compute the number of observations for at least one of your categorical variables.
      *I will be calculating the count of species in each neighbourhood*
```{r}
...
```
   *I made histograms for species diversity to see how it is distributed in neighbourhoods across the city. The one with the bin width of 15 is the best, because the values for species count are pretty spread out, and there is a relatively small amount of neighbourhoods, so the histograms with smaller bin widths look very sparse and make it hard to see any trends.
...
```
    ...
    *Plotting each observation of trees by latitude and longitude to get an idea of distribution within the city, and to see whether or not the different genuses are grouped together.*
```{r}
vantrees_elmmaple %>%
...
3. How has the amount of trees planted in Vancouver changed through time?
    ...
    *I'll make tree planting date into decadal categories*
```{r}
vantrees_decade <-
vantrees_sciname %>%
...
```
    - Create 3 histograms out of summarized variables, with each histogram having different sized bins. Pick the “best” one and explain why it is the best.
    *I'll create histograms for when trees were planted to see change through time*
```{r}
hist10_dateplanted <-
  vantrees_sciname %>%
...
4. How does the planting environment (Root barrier presence, planting area type) impact tree size?
    - Compute the number of observations for at least one of your categorical variables.
    *I'll compute the number of observations for root barrier presence to get an idea of how common it is.*
```{r}
(vancouver_trees %>%
...
```
    ...
    *histogram of tree size by groups of Y/N root barrier*
```{r}
(height_rootbarrier <-
vancouver_trees %>%
  group_by(root_barrier) %>%
...
plot_speciesdiv_neighbourhood <-
  speciesdiv_neighbourhood %>%
  ggplot(aes(x=reorder(neighbourhood_name, +species_count), species_count)) + 
  geom_col() +
  coord_flip()
print(plot_speciesdiv_neighbourhood)
